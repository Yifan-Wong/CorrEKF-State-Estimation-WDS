...
def wntr_2_pd(wn,t_run, dt):
    ...
    links.remove('v2')  # pipe to orifice
    ...
    if has_tanks==1:
        ...
        for j in range(num_tanks):
            ...
            #superjunctions_new['z_inv']=superjunctions_m['z_inv'][superjunctions_m['tank']== True].values[0] # used before
            superjunctions_new['z_inv']=superjunctions_m['z_inv'][tank_ind]
...
#        orifices = orifices.astype(orifices_row.dtypes.to_dict())
    ...
    #Create an orifice connecting the existing tank to the new node    # pipe to orifice
    orifices_row=pd.DataFrame(columns=['name','id','sj_0','sj_1','A','orientation','z_o','y_max','C'],index=[j+1], dtype=object)
    orifices_row['name']='v2'
    orifices_row['id']=7
    orifices_row['sj_0']=int(list(superjunctions_m['name']).index(wn.get_link('v2').start_node_name))
    orifices_row['sj_1']=int(list(superjunctions_m['name']).index(wn.get_link('v2').end_node_name))
    orifices_row['A']=0.164
    orifices_row['orientation']='side'
    orifices_row['z_o']=0
