...
'''considering the relative sensor noise instead of using the same noise'''
import io
import numpy as np
import pandas as pd
...
def run_pipedream_simulation_sensor(inp, t_run=None, dt=None, banded=False, num_iter=40, use_tank_init_cond=False):
    ...
    vals=rng.uniform(0.8, 1.2, (len(superjunctions)))
...
#            j = int(np.floor(model.t/3600)) //2
        ...
        #u_o[1] = 1 # keep tank 1 always open
        ...
        # Tank 1
        if model.H_j[T1_id] < 71.5 + 4 + 0.0:
            u_p[PU1_id] = 1
        if model.H_j[T1_id] > 71.5 + 6.3 - 0.0:
            u_p[PU1_id] = 0
        ...
        if model.H_j[T3_id] < 112.9 + 3 - 0.0:
            u_p[PU4_id] = 1
        if model.H_j[T3_id] > 112.9 + 5.3 - 0.5:
            u_p[PU4_id] = 0
       
        if model.H_j[T3_id] < 112.9 + 1 - 0.0:
            u_p[PU5_id] = 1
        if model.H_j[T3_id] > 112.9 + 3.5 + 0.0:
            u_p[PU5_id] = 0
        ...
        # Tank 5
        if model.H_j[T5_id] < 105.8 + 1.5 - 0:
            u_p[PU8_id] = 1
        ...
        #print('alpha_prv', model._alpha_prv)
    ...
    no_elechange_nodes = superjunctions['tank'] | superjunctions['id'].isin(orifices['sj_1'])
    
    P_df = H_df.values - real_ele.values  # ground truth pressure
    P_df = pd.DataFrame(P_df, index=H_df.index, columns=H_df.columns)

    #max_vals = 200
    pressure_sensor_std_dev = 0.5
    tank_sensor_std_dev = 0.2
    rng = np.random.default_rng(0)
    pressure_noise = rng.normal(0, pressure_sensor_std_dev, size=H_df.shape)
    tank_noise = rng.normal(0, tank_sensor_std_dev, size=H_df.shape)
    
    no_elechange_cols = no_elechange_nodes[no_elechange_nodes].index
    P_df_sensor = P_df.copy()
    P_df_sensor.iloc[:,no_elechange_cols] = P_df.iloc[:,no_elechange_cols] + tank_noise[:, no_elechange_cols]
    # adding more noise to pressure sensor only
    elechange_cols = no_elechange_nodes[~no_elechange_nodes].index
    P_df_sensor.iloc[:,elechange_cols] = P_df.iloc[:,elechange_cols] + pressure_noise[:, elechange_cols]

    H_df_sensor = P_df_sensor.values + model_ele.values
    H_df_sensor = pd.DataFrame(H_df_sensor, index=H_df.index, columns=H_df.columns)
    # make sure the tank level (after adding noise) do not exceed the max and min limit
    tank_cols = superjunctions['tank'][superjunctions['tank']].index
    tank_col_names = superjunctions['name'].iloc[tank_cols].values
    H_df_sensor.loc[:, tank_col_names] = H_df_sensor.loc[:, tank_col_names].clip(lower=tank_min[tank_cols],upper=tank_max[tank_cols],axis=1)
    
    # head_sensor_std_dev = 0.2
    # rng = np.random.default_rng(4)
    # vals = rng.normal(0, head_sensor_std_dev, size=H_df.shape)
    ...
    flow_sensor_std_dev = 0.002
    rng = np.random.default_rng(4)
    vals = rng.normal(0, flow_sensor_std_dev, size=Q_df.shape)
    Q_df_sensor = Q_df+vals
    ...
    return H_df, Q_df, H_df_sensor, Q_df_sensor, Q_o, Q_pump, Q_prv, model, Q_in_all_df , pumps, superjunctions, orifices, superlinks, prvs, flip_mult
